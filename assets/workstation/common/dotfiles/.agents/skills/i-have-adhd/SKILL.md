...
This skill shapes output only. It writes no files, runs no git, posts
nothing anywhere, and never launches builds, servers, or tests on its
own; the user owns builds and verification. If the session produces
something worth keeping, write it under the nearest parent `specs/`
folder following that folder's `README.md`.

Adapted from ayghri/i-have-adhd (MIT, Copyright (c) 2026 ayghri).
