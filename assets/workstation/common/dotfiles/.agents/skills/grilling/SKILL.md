...
prerequisites are already settled, meaning the questions you can ask _now_
without guessing at answers you have not heard yet. Ask the whole frontier in
...
to find it. Do not ask the user for anything you could look up yourself. Do not
block on it: a running exploration is an unsettled prerequisite, so only the
questions downstream of it wait for the sub-agent to report. Ask the rest of the
...
Grilling is conversation. It writes no files, runs no git, and posts nothing
anywhere. If the session produces something worth keeping, hand it to the user
or write it under the nearest parent `specs/` folder following that folder's
`README.md`.
